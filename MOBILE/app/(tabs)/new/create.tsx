...
type Step = 'basics' | 'logistics' | 'requirements' | 'pricing' | 'review' | 'images';
...
function CreateJobScreen() {
  const { canPostJob, isAdmin, isEntrepreneur, isTechnician } =
    usePermissions();
  const { draftMission, updateDraftMission, createMission, loading, error } = useMissionStore();
  ...
  const calculateBasePrice = () => {
    const baseRate = draftMission?.needed_actor === 'technician' ? 25000 : 15000;
  ...
  // Update your adjustment price calculation
  const calculateAdjustments = () => {
    let adjustment = 0;
    const workers = draftMission?.needed_actor_amount || 1;
  
    // Equipment adjustment (positive value)
    if (providesEquipment) {
      adjustment += 5000 * workers;
    }
    ...
    // Ensure adjustment price never goes below 0
        ...
        <View style={[styles.inputContainer, { flex: 1 }]}>
          ...
          >
            ...
            <TextInput
              ...
              onChangeText={(text) => updateDraftMission({ start_date: text })}
        ...
        <View style={[styles.inputContainer, { flex: 1 }]}>
          ...
          >
            ...
            <TextInput
              ...
              onChangeText={(text) => updateDraftMission({ end_date: text })}
              placeholder="JJ/MM/AAAA"
              placeholderTextColor={colors.muted}
            />
          </View>
          ...
          {EXPERIENCE_LEVELS.map((level) => (
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                updateDraftMission({
                  required_experience_level: level.id as ExperienceLevel || 'starter',
                });
              }}
            >
              <Text
            ...
            {adjustments !== 0 && (
              <View style={styles.priceRow}>
                ...
                <Text
                  style={[
                    styles.priceValue,
                    { color: adjustments < 0 ? colors.success : colors.error },
                  ]}
                >
                  {adjustments > 0 ? '+' : ''}
              ...
              <View style={styles.priceRow}>
                ...
                <Text
                  style={[
                    ...
                    {
                      color:
                        priceAdjustment < 0 ? colors.success : colors.error,
                    },
                  ]}
                >
                  {priceAdjustment > 0 ? '+' : ''}
      ...
      <Animated.View entering={FadeInDown}>
        <View style={styles.section}>
          ...
          <Text style={[styles.label, { color: colors.muted }]}>
            You can select up to 4 images (max size: 5MB each).
          </Text>
